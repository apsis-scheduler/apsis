...
class ECSTaskManager:
    ...
    async def start_task(
        ...
        # Higher retries and delay to handle instance refresh scenarios
        max_retries = 10
        base_delay = 15
        ...
        for attempt in range(max_retries + 1):  # +1 for initial attempt
            try:
                ...
                if container_override:
                    container_override["name"] = self.container_name
                ...
                actual_disk_space = (
                    ...
                    else self.default_disk_space_gb
                )

                volume_configurations = self._create_ebs_volume_config(
                    actual_disk_space
                )
                ...
                is_retryable = (
                    "RESOURCE:MEMORY" in error_message
                    or "RESOURCE:CPU" in error_message
                    or "TaskCreationFailed" in error_message
                    # No container instances available (e.g., during instance refresh)
                    or "No Container Instances" in error_message
                )

                # If this is the last attempt or error is not retryable, raise the exception
                if attempt >= max_retries or not is_retryable:
                    logger.error(
                        f"Failed to start ECS task after {attempt + 1} attempts: {e}"
                    )
                    raise
    ...
    async def wait_for_task_status(
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout:
            ...
            should_report = (status != last_status) or (
                elapsed > 0 and int(elapsed) % 30 == 0
            )
    ...
    async def get_task_logs(
        ...
        try:
            ...
            logger.warning(
                f"Could not retrieve task logs from stream '{log_stream_name}': {e}"
            )
            return []
