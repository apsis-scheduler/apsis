    ...
    procstar:
      agent:
        ecs:
          ...
          task_startup_timeout: 900
          default_disk_space_gb: 20
...
- ``task_startup_timeout``: Maximum time in seconds to wait for an ECS task to
  reach the RUNNING state. If the task doesn't start within this time, the run
  transitions to error.

- ``default_disk_space_gb``: Default ephemeral EBS storage size in GiB for ECS
